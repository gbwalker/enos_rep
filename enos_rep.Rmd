...
dat.confed <- read.csv("confederate_face_data.csv")
dat.hisp <- read.csv("hispanic_face_data.csv")
dat.white <- read.csv("white_face_data.csv")
```
...
# A kable for later?
# final.mat = as_tibble(final.mat)
# colnames(final.mat) =
# final.mat <- final.mat %>%
#   mutate(ate = as.numeric(ate),
#          greater.p.value = as.double(greater.p.value),
#          lesser.p.value = as.double(lesser.p.value),
#          x.sd = as.double(x.sd),
#          x.mean = as.double(x.mean))
# kable(final.mat, format = "markdown", digits = 4)
```
...
# reuses the final.mat variable and creates new subset
# names.
final.mat <- matrix(nrow = 0, ncol = 8)
subsets <- c("ta", "tb")
...
Ns <- c(
  "N", max(final.mat.redact$N[final.mat.redact$subset == "all"]),
...
# Ns <- c(
#   "N", max(final.mat.redact$N[final.mat.redact$subset == "all.prime"]),
#   max(final.mat.redact$N[final.mat.redact$subset == "all"])
# )
# 
# h1 <- c("", paste("(", 1:2, ")", sep = ""))
...
xtable.out.balance.text <- xtable(out.balance.test, caption = "Difference in means between the two samples", digits = 2)
